...
def experience_section(experience_data):
    ...
    for job_title, (job_description, image) in experience_data.items():
        ...
        expander.image(image, width=200)
...
def skills_section(programming_data, spoken_data):
    ...
    for spoken, proficiency in spoken_data.items():
        st.write(f"{skill}{info.programming_icons.get(spoken, "")}: {proficiency}")
    st.write("---")
skills_section(info.programming_data, info.spoken_data)
#Activities
def activities_section(leadership_data, activities_data):
    st.header("Activities")
    tab1, tab2 = st.tabs(["Leadership", "Activities"])
    with tab1:
    ...
    with tab2:
        ...
        for title, details in activities_data.items():
            expander = st.expander(f"{title}")
            for bullet in details:
                expander.write(bullet)
    st.write("---")
